...
M.get_current_mode = function(self)
  ...
  local modestr = ''
  -- truncated (small window)
  if self:is_truncated(self.trunc_width.mode) then
    max_mode_length = 3
    modestr = self.modes[current_mode][2]
  else
    -- not truncated
    max_mode_length = 9
    modestr = self.modes[current_mode][1]
  end
  -- insert modestr into string so it has constant length
  return string.format(' %s '..string.rep(' ', max_mode_length - modestr:len()), modestr):upper()
end
